...
namespace GhostFrenzyGame
{
	...
	/// </summary>
	public class GFGEnemy:MonoBehaviour
	{
		...
		void Update()
		{
			...
			if ( isSpawning == false )    
			{
                ...
                //Vector3 newDir = Vector3.RotateTowards(thisTransform.forward, targetPosition - thisTransform.position, 0.5f * Time.deltaTime, 0.0F);
			...
			if( isSpawning == false && other.tag == touchTargetTag )
			{
				...
				foreach( var touchFunction in touchFunctions )
				{
					...
					if( touchFunction.functionName != string.Empty )
					{
						...
						{
							// Run the function
							other.SendMessage(touchFunction.functionName, transform, SendMessageOptions.DontRequireReceiver);
						}
